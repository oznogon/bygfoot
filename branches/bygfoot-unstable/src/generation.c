...
void give_player_names(gint first_team, gint last_team)
{
    gint i, k, name_start;
...
void choose_european_names_uefa(gchar all_names[][50],
...
/* choose a team name from a given country which isn't
   yet on the list */
void choose_european_name(gchar all_names[][50],
...
void choose_european_names(gchar european_names[][50], gchar all_names[][50])
{
    gint i;
    gint order[15];
...
void generate_players(gint team_id)
{
    gint i;
...
void fill_in_stadium(gint team_id)
{
...
void give_teams_names(gboolean only_euro)
{
    gint i, j = 0;
...
/* fill in all the data for a team: name, players,
   stadium etc. */
void fill_in_team(gint team_id)
{
    gint i;
...
void country_names(gint country_number, const gchar *country_filename)
{
