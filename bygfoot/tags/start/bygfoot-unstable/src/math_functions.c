...
gfloat gauss_dist(gfloat a, gfloat b, gfloat c, gfloat d)
{						   
    gfloat result;				   
...
/* write a permutation of the integers between 'min' and 'max'
   into the array 'perm' */
gint write_permutation(gint *perm, gint min, gint max)
{
    /*c*/
    gdouble x[max - min + 1];
    gint i, j;
...
/* sum up the elements of an integer array between indices
   'min' and 'max' */
gint sum(gint *array, gint min, gint max)
{
    gint i;
    gint result = 0;
...
/* compute the smallest element of an array from element
 first to element last */
gint min_int(gint *array, gint first, gint last)
{
    gint i, result=array[first];
...
gfloat min_float(gfloat *array, gint first, gint last)
{
    gint i;
    gfloat result=array[first];
...
gint sort_float_array(gfloat *array, gint *order, 
...
gint get_place(gint value, gint place)
{
    if(place < 10)
