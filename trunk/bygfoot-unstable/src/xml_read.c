...
/* Read the XML save file and set all variables */
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_save: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}
	cur = cur->next;
...
read_xml_options(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_options: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}		
...
read_xml_options_memory(xmlDocPtr doc, xmlNodePtr root, gint memory_number)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_options_memory: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}
	cur = cur->next;		
    }
}
...
read_xml_goal(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_goal: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}
	cur = cur->next;
    }
    return my_goal;
...
read_xml_transfer(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_transfer: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}		
...
read_xml_stadium(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_stadium: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}
...
read_xml_team(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_team: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}						
...
read_xml_player(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_player: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}
...
read_xml_player_stat(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_player_stat: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}
	cur = cur->next;
    }
...
read_xml_season_stat(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_season_stat: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}		
	cur = cur->next;
    }
...
read_xml_history(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_history: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}				
	cur = cur->next;
    }
}
...
read_xml_fixture(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if (!xmlStrcmp(cur->name, (const xmlChar *) "result"))
	{
		...
		if (!known_node2)
		{
		    fprintf(stderr, "read_xml_fixture2: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur2->name);
		}				
		cur2 = cur2->next;
	    }
	    result_index++;
	...
	if (!known_node)
	{
	    fprintf(stderr, "read_xml_fixture: Warning! XML node %s is not known. Are you sure you use the good bygfoot version?\n", cur->name);
	}
	cur = cur->next;
    }
    return my_fixture;
