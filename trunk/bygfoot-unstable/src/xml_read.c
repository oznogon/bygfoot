...
gint
check_xml_save(gchar * file_name)
{
...
/* Read the XML save file and set all variables */
{
	...
	if (!known_node)
	{
	    fprintf(stderr, "Warning! XML node \'%s\' is not known. Are you sure you use the correct bygfoot version?\n", cur->name);
	}
	cur = cur->next;
